...
$id = $_GET['Id'];
  echo $id;
    $info = $ModeloComentarios->verIdComentario($id);
         ...
         <div class="card card-primary">
          <div class="titulos">
              <div class="card-header">
                <h3 class="card-title">Eliminar Comentario</h3>
              </div>
         </div>
              <!-- /.card-header -->
              <!-- form start -->
              
              <form action="../controlador/delatecomentario.php" method="POST">
              
            <input type="hidden" name="id" value="<?= $id?>">
            
            
            <label>persona que hizo el comentario </label><br>
            <?php 
                if ($info != null) {
                    foreach ($info as $infoComentario) {  
                        echo $infoComentario['nombre']."<br>";
                    }
